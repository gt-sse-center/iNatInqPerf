...
def test_faiss_ivfpq_train_and_search_with_large_training(ivfpq_trainset, small_data):
    train_ids, X_train = ivfpq_trainset
    ids, X = small_data

    be = FaissIVFPQ()
    # Use fewer PQ centroids (nbits=4 -> 16) so 300 training points suffice without warnings
    be.init(dim=2, metric="ip", nlist=2, m=1, nbits=4, nprobe=2)
...
def test_faiss_ivfpq_l2_metric_delete_and_drop(ivfpq_trainset, small_data):
    ...
    be.init(dim=2, metric="l2", nlist=2, m=1, nbits=4, nprobe=2)
    _buf = io.StringIO()
    with contextlib.redirect_stderr(_buf):
        be.train(X_train)
    be.upsert(ids, X)
...
def test_faiss_ivfpq_runtime_nprobe_override_and_upsert_replace(ivfpq_trainset, small_data):
    ...
    be = FaissIVFPQ()
    be.init(dim=2, metric="ip", nlist=2, m=1, nbits=4, nprobe=1)
    _buf = io.StringIO()
    with contextlib.redirect_stderr(_buf):
        be.train(X_train)
...
def test_metric_mapping_and_unwrap_real_index(ivfpq_trainset, small_data):
    ...
    ivf = fb_mod._unwrap_to_ivf(be.index.index)
...
def test_faiss_ivfpq_cosine_metric_and_topk_gt_ntotal(ivfpq_trainset, small_data):
    ...
    ids, X = small_data
    be = FaissIVFPQ()
    be.init(dim=2, metric="cosine", nlist=2, m=1, nbits=4, nprobe=2)
    _buf = io.StringIO()
    with contextlib.redirect_stderr(_buf):
        be.train(ivfpq_trainset[1])
    be.upsert(ids, X)
...
#     be.init(dim=2, metric="ip", nlist=64, m=1, nbits=2)  # nprobe defaults to 32
